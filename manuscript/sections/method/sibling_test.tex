...
parent leaf count. The diagnostic estimator uses the working post-selection scale family
...
and the implementation raises an error.
...
\right)
\end{equation}
is not a validated calibrated p-value. The production path preserves the raw
statistic, raw fixed-reference p-value, scale estimate, support diagnostics,
and dependency groups, but positive-dimensional decisions have status
\texttt{undefined\_unvalidated\_reference\_law} and do not enter sibling FDR.
